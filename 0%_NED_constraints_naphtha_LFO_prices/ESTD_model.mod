...
param share_NG_NED >= 0, <= 1; #share of natural gas in the non energy demand based on historical share (FPS stat)
...
#########################################
#Case specific variable definitions
var Low_wood >= 0 ;
var High_wood >= 0 ;
...
subject to low_wood_definition:
       Low_wood = sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t ["WOOD2", h, td] + F_t ["WOOD3", h, td] + F_t ["WOOD5", h, td] + F_t ["WOOD7", h, td] + F_t ["WOOD9", h, td]);
 

subject to high_wood_definition:
       High_wood = sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t ["WOOD1", h, td] + F_t ["WOOD4", h, td] + F_t ["WOOD6", h, td] + F_t ["WOOD8", h, td] + F_t ["WOOD10", h, td]);

 
subject to low_to_high_limitation:
       Low_wood <= 0.4 * High_wood / 0.6 * (18.7/20.1);
...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		...
		else (if l == "NON_ENERGY_NaG" then
			end_uses_input["NON_ENERGY"] * share_NG_NED / total_time
		else (if l == "NON_ENERGY_LFO" then
			end_uses_input["NON_ENERGY"] * (1 - share_NG_NED) / total_time
		else 
...
F ["DHN"] = F["GASIFICATION_SNG"]*layers_in_out["GASIFICATION_SNG","HEAT_LOW_T_DHN"] +  sum {j in TECHNOLOGIES_OF_END_USES_TYPE["HEAT_LOW_T_DHN"]} (F [j]);
