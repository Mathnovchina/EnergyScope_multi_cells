...
def parse_args():
    ...
    p.add_argument("--simplex", action="store_true",
                   help="Use dual simplex instead of barrier (gives clear infeasibility diagnosis)")
    return p.parse_args()
...
def main():
    ...
    if args.simplex:
        print("  [SIMPLEX MODE] Using dual simplex instead of barrier")
        cplex_opts = ['lpmethod 2',
                      'predual=-1',
                      'timelimit 172800',
                      'display=2']
        ampl_opts = {'show_stats': 3,
                     'log_file': str(run_dir / 'log.txt'),
                     'presolve': 200,
                     'times': 1,
                     'gentimes': 1,
                     'cplex_options': ' '.join(cplex_opts)}
        my_model.set_esom(ampl_path=ampl_path_arg, ampl_options=ampl_opts)
    else:
