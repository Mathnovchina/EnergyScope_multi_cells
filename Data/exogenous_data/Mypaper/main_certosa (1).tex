...
\title{From Bioeconomy Promise to Biodiversity Ceiling: The Role of Forest Biomass in Finnish Decarbonisation\footnote{PhD student at University of Pisa}\footnote{mail. m.bordenave@student.unisi.it}\footnote{\url{https://orcid.org/0000-0002-5571-5877}}}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{llrrr}
    \toprule
    \textbf{Forest scenario} & \textbf{Policy case} & \textbf{Cost} & \textbf{CO$_2$ net} & \textbf{Wood used} \\
    \midrule
...
\begin{figure}[H]
  ...
  \caption{Biomass allocation by final use in Finland 2035. Each bar
  shows the total biomass feedstock attributed to its downstream
  service: methane produced through biomethanation
  (\texttt{BIOMETHANATION\_WET\_BIOMASS}, \texttt{\_BIOWASTE}) and
  through Fischer--Tropsch gas synthesis (\texttt{BIOMASS\_TO\_METHANE},
  \texttt{BIOWASTE\_TO\_METHANE}) is re-attributed to its actual
  downstream consumer (gas-fired CCGT power, decentralised gas
  boilers, gas-fuelled buses and freight ships, etc.) in proportion
  to that consumer's share of total \texttt{GAS}-layer demand. The
  italic label above each bar reports the woody (forest \texttt{WOOD})
  share of total biomass; the residual is mostly municipal
  biowaste/waste in the unconstrained cases. The figure highlights
  how the no-nuclear cases increase pressure on biomass in S1 and
  S2, whereas S3/BDS remains capped by its conservative forest
  ceiling and reallocates more selectively.}
...
First, roughly 27.9~TWh is directed to high-value-chemicals production and approximately 0.8~TWh to methanol synthesis across all nine scenarios -- a near-constant baseline driven by exogenous non-energy industrial demand that the optimiser cannot reroute. Because this commitment is met largely from forest \texttt{WOOD}, it is the main reason that the woody share of total biomass already exceeds 60\% even in the unconstrained cases (71\%, 67\%, 63\% in S1, S2, S3). It also means that the full S3/BDS ceiling of 40~TWh retains only about 12~TWh for heat or transport use after chemicals are satisfied.

Second, the -95\% GHG constraint triggers a qualitative shift in S1/BES and S2/NFS. In the unconstrained cases the residual biomass after chemicals is directed almost entirely to industrial process-heat boilers (35.6~TWh in S1, 26.5~TWh in S2 and 19.5~TWh in S3, of which roughly 11~TWh comes from the municipal-waste boiler in all three), with a marginal 1~TWh in decentralised heat. Under the climate target, approximately 21.5~TWh of the available budget in S1 and S2 is instead converted to mobility fuels -- the hardest-to-electrify end uses, including biomass-to-diesel for road and aviation as well as the share of biomethane and Fischer--Tropsch gas that is consumed by CNG buses and gas-fuelled freight shipping -- while industrial-heat allocation remains broadly stable in the wood component. In S3/BDS, the 40~TWh ceiling is already fully committed to chemicals (28.7) and industry heat (28.8) under the climate constraint, leaving only about 1.5~TWh of residual room for mobility fuels; the rest of the transport sector is decarbonised through imported renewable diesel and electrification, substituting the domestic biomass-to-fuel conversion route with higher-cost pathways.

Third, removing nuclear in S1/BES and S2/NFS amplifies biomass pressure across multiple services simultaneously. Total biomass jumps from 85~TWh to 119~TWh in S1 and to 115~TWh in S2, and the woody share rises to 90--91\%. Industrial heat absorbs an additional 10.1~TWh (S1, from 35.1 to 45.2~TWh) and 8.7~TWh (S2, 35.2$\rightarrow$43.9~TWh); decentralised building-heat boilers take a new 15.6~TWh (S1) and 12.4~TWh (S2); and a further 3.1--3.4~TWh of biomass-derived methane is re-routed to gas-fired electricity production via CCGT, while mobility fuels grow by another 5~TWh on top of the with-nuclear level (21.5$\rightarrow$26.7~TWh). In other words, biomass becomes a multi-service strategic resource simultaneously filling gaps in building heat, industrial heat, electricity, and transport fuels. S3/BDS cannot follow this pattern: its end-use breakdown is structurally identical between the -95\% and the no-nuclear case (58.9~TWh in both), because the 40~TWh domestic ceiling is already binding under the weaker constraint. The additional decarbonisation burden -- decentralised heat, gas-fired flexible power -- must be absorbed entirely by non-biomass pathways, which is why the cost penalty for nuclear removal is roughly thirteen times larger in S3 than in S2 ($+1{,}438$ vs $+105$~MEUR/y above the S2 unconstrained reference).

The policy implication is that the debate is not only about the total quantity of forest biomass available, but also about which end uses receive priority when both climate ambition and forest constraints tighten simultaneously. In S1 and S2 the model uses biomass as a strategic balancing resource the moment zero-carbon baseload flexibility declines; S3/BDS cannot respond to the same pressure by expanding forest harvest, so it is forced to reallocate scarce biomass more selectively and lean more heavily on non-biomass substitutions elsewhere in the system.
